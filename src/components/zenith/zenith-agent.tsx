...
export function ZenithAgent() {
  ...
  if (!open) return <button onClick={() => setOpen(true)} className="fixed bottom-6 right-6 z-50 w-14 h-14 rounded-full bg-brand flex items-center justify-center shadow-lg shadow-brand/25 hover:scale-105 active:scale-95 transition-all" title="Zenith Agent"><Sparkles className="w-6 h-6 text-white" /></button>;

  return <div className={cn("fixed bottom-6 right-6 z-50 transition-all duration-300 ease-out", minimized ? "w-14 h-14" : "w-[380px] h-[520px]")}>{minimized ? <button onClick={() => setMinimized(false)} className="w-14 h-14 rounded-full bg-brand flex items-center justify-center shadow-lg"><Sparkles className="w-6 h-6 text-white" /></button> : <div className="flex flex-col h-full rounded-xl border border-border bg-surface shadow-2xl overflow-hidden">
